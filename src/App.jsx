...
import { 
  ...
  Tooltip as RechartsTooltip, Legend, PieChart, Pie, Cell, LabelList 
} from 'recharts';
...
// ==========================================
// Para producción (GitHub Pages/Vercel), debes crear un proyecto gratuito en Firebase 
// (firebase.google.com) y pegar aquí las credenciales. 
// Si dejas esto vacío, el sistema funcionará perfectamente en "Modo Local" (LocalStorage).
            ...
            <div className="grid grid-cols-2 gap-2">
              {['Jefe', 'Coordinador', 'Especializado', 'Junior'].map(role => (
                <button
                  key={role}
                  type="button"
                  onClick={() => { setSelectedRole(role); setSelectedUser(''); }}
...
const JuniorDashboard = ({ user, tasks, categories, onAddTask, onUpdateTaskStatus, onUpdateTaskData }) => {
  ...
  const handleSubmitReport = (e) => {
    ...
    onUpdateTaskData(activeTask.id, {
      ...
      status: requestContinuation ? STATUS.SOLICITUD_CONTINUIDAD : STATUS.EN_REVISION
    });
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index, value }) => {
    ...
    const y = cy + radius * Math.sin(-midAngle * Math.PI / 180);
    return (
      <text x={x} y={y} fill={index === 0 ? "white" : "#4B5563"} textAnchor="middle" dominantBaseline="central" fontSize={14} fontWeight="bold">
        {value}
      </text>
    );
  };
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 border-t-8 border-t-[#8CC63F] flex flex-col md:flex-row gap-6 items-center">
         ...
         <div className="w-full md:w-1/3 h-[180px] flex items-center justify-center">
            {totalTasks > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} innerRadius={40} outerRadius={80} dataKey="value" stroke="none" labelLine={false} label={renderCustomLabel}>
                    {pieData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
                  </Pie>
                  <RechartsTooltip />
                  <Legend verticalAlign="bottom" height={20} wrapperStyle={{fontSize: '11px', fontWeight: 'bold'}}/>
                </PieChart>
              </ResponsiveContainer>
            ) : (
...
const ReviewerDashboard = ({ user, tasks, categories, employees, onAddTask, onUpdateTaskStatus, onAddComment, onUpdateTaskData }) => {
  ...
  const handleReview = (decisionStatus) => {
    ...
    if (decisionStatus === STATUS.CONTINUADO) {
      ...
      onUpdateTaskStatus(activeTask.id, STATUS.CONTINUADO);
    } else if (decisionStatus === STATUS.CUMPLIDO) {
       onUpdateTaskStatus(activeTask.id, STATUS.CUMPLIDO);
       if (activeTask.category === categories[0].name && activeTask.subcategory !== 'Socialización') {
          const nextWeek = getNextWeekData(activeTask.assignedWeek);
          const socTask = {
            id: Date.now().toString() + '-soc',
            title: 'Socialización: ' + activeTask.title,
            description: 'Realizar la socialización del documento actualizado y aprobado.',
            category: activeTask.category,
            subcategory: 'Socialización', 
            deadlineInfo: 'Sin límite estricto',
            assignedWeek: nextWeek,
            assigneeId: activeTask.assigneeId,
            reviewerId: activeTask.reviewerId,
            status: STATUS.ASIGNADO,
            createdAt: getCurrentDateFormatted(),
            comments: [],
            managementDescription: '',
    ...
    <div className="space-y-6">
                    ...
                    {assigneeTasks.map(task => (
                      <div key={task.id} className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden">
                        <div className="p-4 flex-1 flex flex-col">
                          ...
                          <h4 className="font-bold text-gray-800 text-sm mb-1">{task.title}</h4>
                          {task.subcategory && <p className="text-xs text-[#8CC63F] font-bold mb-2">{task.subcategory}</p>}
                          
                          <div className="flex items-center text-xs text-gray-500 font-medium gap-1 mt-auto pt-3 border-t border-gray-50">
                            <Icon name="clock" className="w-4 h-4 text-[#AAB4C2]" /> Límite: {task.deadlineInfo}
                            {(task.continuedCount > 0) && <span className="ml-2 font-bold text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded border border-orange-200">Semana {task.continuedCount + 1}</span>}
                          </div>
                        </div>
                        <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex flex-wrap gap-2">
                          <button onClick={() => { setActiveTask(task); setIsReviewModalOpen(true); setReviewComments(''); setErrorMsg(''); }} className="text-xs flex-1 bg-[#165399] hover:bg-[#114078] text-white px-3 py-2 rounded-lg font-bold flex items-center justify-center gap-1 shadow-sm transition-colors">
                             <Icon name="check" className="w-4 h-4" /> {(task.status === STATUS.EN_REVISION || task.status === STATUS.SOLICITUD_CONTINUIDAD) ? 'Evaluar' : 'Ver Estado'}
                          </button>
                          <button onClick={() => openEditModal(task)} className="text-xs flex-1 bg-white border border-gray-300 text-gray-700 hover:bg-gray-100 px-3 py-2 rounded-lg font-bold flex items-center justify-center gap-1 shadow-sm transition-colors">
              ...
              <div className="pt-4 border-t border-gray-200">
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-3">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto flex-wrap">
                    ...
                    <button onClick={() => handleReview(STATUS.CONTINUADO)} className="px-4 py-2.5 border border-orange-400 bg-orange-50 text-orange-700 rounded-lg hover:bg-orange-100 font-bold text-sm transition-colors shadow-sm flex-1 sm:flex-none">Continuar Próx. Sem</button>
      ...
      <Modal isOpen={isAssignModalOpen} onClose={() => setIsAssignModalOpen(false)} title="Asignar Entregable a Profesional">
        <form onSubmit={handleCreateTask} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-bold text-[#165399] mb-1">Semana de Ejecución <span className="text-red-500">*</span></label>
            <input type="text" value={formData.week} onChange={e => setFormData({...formData, week: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#165399] outline-none" />
          </div>
          <div>
            <label className="block text-sm font-bold text-[#165399] mb-1">Descripción / Objetivos Específicos</label>
            <textarea rows="3" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#165399] outline-none"></textarea>
...
const App = () => {
  ...
  useEffect(() => {
    if (!auth) {
      // Modo Local: Sin Firebase, procedemos inmediatamente
      setIsDbReady(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (db && auth?.currentUser) {
      ...
      // Firebase Inactivo: Cargar desde LocalStorage del navegador
  ...
  // Controladores BD (Multi-entorno)
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {dashboardMode === 'ADMIN' ? (
          ...
          <>
            ...
            {currentUser.canReview ? (
              <ReviewerDashboard user={currentUser} tasks={tasks} categories={appConfig.categories} employees={appConfig.employees} onAddTask={handleAddTask} onUpdateTaskStatus={handleUpdateTaskStatus} onAddComment={handleAddComment} onUpdateTaskData={handleUpdateTaskData} />
            ) : (
              <JuniorDashboard user={currentUser} tasks={tasks} categories={appConfig.categories} onAddTask={handleAddTask} onUpdateTaskStatus={handleUpdateTaskStatus} onUpdateTaskData={handleUpdateTaskData} />
            )}
          </>
        )}
      </main>
