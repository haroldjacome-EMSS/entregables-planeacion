      ...
      <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg border-t-8 border-[#165399]">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            ...
            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ejemplo@emssanar.com" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#165399] outline-none transition-shadow bg-gray-50 text-gray-800 font-medium" />
          </div>
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(json)) {
          if(window.confirm(`Se encontraron ${json.length} registros. ¿Desea importarlos a la base de datos?`)) { onImportTasks(json); alert(`Importación iniciada.`); }
        } else { alert("Formato incorrecto (debe ser array JSON)."); }
            ...
            <div className="space-y-4">
              <div className="flex flex-col md:flex-row gap-3 mb-4 bg-gray-50 p-4 rounded-lg border border-gray-200 items-center justify-between">
                <div className="flex gap-2">
                  <button onClick={exportToJSON} className="bg-[#165399] hover:bg-[#114078] text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm flex items-center gap-1"><Icon name="download" className="w-4 h-4"/> Backup (Exportar JSON)</button>
                  <label className="bg-[#8CC63F] hover:bg-[#78b030] text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm cursor-pointer flex items-center gap-1"><Icon name="upload" className="w-4 h-4"/> Restaurar (Importar JSON)<input type="file" accept=".json" onChange={handleFileUpload} className="hidden" /></label>
                </div>
                <button onClick={() => { if(window.confirm('🚨 ¡PELIGRO! ¿ELIMINAR TODOS los entregables? Acción destructiva.')) onClearAllTasks(); }} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm flex items-center gap-1"><Icon name="trash" className="w-4 h-4"/> Vaciar Base de Datos Completa</button>
              </div>

              <div className="overflow-x-auto border border-gray-200 rounded-lg max-h-[500px]">
                <table className="min-w-full divide-y divide-gray-200 relative">
                  <thead className="bg-gray-100 sticky top-0 z-10 shadow-sm">
                    <tr><th className="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">ID / Fecha</th><th className="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Título</th><th className="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Profesional</th><th className="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Semana</th><th className="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Estado</th><th className="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase">Acción Cruda</th></tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {(tasks || []).map(task => (
                      <tr key={task.id} className="hover:bg-orange-50 transition-colors">
                        <td className="px-4 py-2 text-xs text-gray-500"><span className="font-mono text-[9px] block text-gray-400">{task.id}</span>{task.createdAt}</td>
                        <td className="px-4 py-2 text-xs font-bold text-gray-900 max-w-xs truncate" title={task.title}>{task.title}</td>
                        <td className="px-4 py-2 text-xs text-gray-700">{task.assigneeId}</td>
      ...
      <Modal isOpen={isEditUserModalOpen} onClose={() => setIsEditUserModalOpen(false)} title="Editar Miembro del Equipo">
        {editingUser && (
          <form onSubmit={handleUpdateUser} className="space-y-4">
            ...
            <div className="pt-4 flex justify-end gap-2"><button type="button" onClick={() => setIsEditUserModalOpen(false)} className="px-4 py-2 bg-gray-100 rounded-lg font-bold">Cancelar</button><button type="submit" className="px-4 py-2 bg-[#165399] text-white rounded-lg font-bold">Guardar</button></div>
          </form>
        )}
      </Modal>
      ...
      <Modal isOpen={editDbModal} onClose={() => setEditDbModal(false)} title="Edición Cruda de Registro">
        {dbTaskEdit && (
          <form onSubmit={saveDbTaskEdit} className="space-y-4">
            ...
            <div className="pt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditDbModal(false)} className="px-4 py-2 bg-gray-100 rounded text-sm font-bold">Cancelar</button><button type="submit" className="px-4 py-2 bg-orange-600 text-white rounded text-sm font-bold">Forzar Guardado</button></div>
          </form>
        )}
      </Modal>
...
const JuniorDashboard = ({ user, tasks, categories, onAddTask, onUpdateTaskStatus, onUpdateTaskData }) => {
  ...
  const handleCreateTask = (e) => {
    ...
    onAddTask({
      ...
      category: selectedCategory?.name || 'General', subcategory: formData.categoryId === 1 ? formData.subcategory : '',
  ...
  const canRequestContinuation = activeTask && (!((activeTask.category === (categories && categories[0]?.name) && taskSubcategoryObj?.maxWeeks) ? (activeTask.continuedCount + 1) >= taskSubcategoryObj.maxWeeks : false) || activeTask.allowExtraTime);
  ...
  useEffect(() => {
    ...
    const unsubConfig = onSnapshot(configRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Aquí agregamos los valores de respaldo para evitar la pantalla blanca
