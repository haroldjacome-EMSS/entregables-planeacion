            ...
            <div className="space-y-6">
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                ...
                <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1">Correo (Auth)</label>
                    <input required type="email" value={newUser.email} onChange={e=>setNewUser({...newUser, email: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded focus:ring-[#165399] outline-none text-sm"/>
                  </div>
                  <div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Correo (Firebase Auth)</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {config.employees.map(emp => (
                      <tr key={emp.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-[#165399] font-bold">{emp.email || 'SIN CORREO'}</td>
      ...
      <Modal isOpen={isEditUserModalOpen} onClose={() => setIsEditUserModalOpen(false)} title="Editar Miembro del Equipo">
        {editingUser && (
          <form onSubmit={handleUpdateUser} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-bold text-[#165399] mb-1">Correo Institucional (Firebase Auth)</label>
              <input required type="email" value={editingUser.email || ''} onChange={e => setEditingUser({...editingUser, email: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#165399] outline-none" />
            </div>
            <div>
...
const App = () => {
  const [firebaseUser, setFirebaseUser] = useState(null); // Usuario autenticado por Google
  ...
  useEffect(() => {
    ...
    const unsubConfig = onSnapshot(configRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Fusión de seguridad: Si la DB vieja no tenía los correos, los fusiona desde DEFAULT_EMPLOYEES temporalmente
        ...
        setAppConfig({ ...dbData, employees: mergedEmployees });
      } else {
        setDoc(configRef, { employees: DEFAULT_EMPLOYEES, categories: DEFAULT_CATEGORIES });
        setAppConfig({ employees: DEFAULT_EMPLOYEES, categories: DEFAULT_CATEGORIES });
      }
    }, (err) => console.error(err));
  ...
  if (!isDbReady) {
    return <div className="min-h-screen bg-gray-100 flex items-center justify-center font-bold text-[#165399]">Cargando plataforma segura...</div>;
  }
  ...
  const handleAddTask = async (t) => { 
    if (db) await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'emssanar_tasks', t.id), t); 
  };

  const handleUpdateTaskStatus = async (id, s) => { 
    if (db) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'emssanar_tasks', id), { status: s }); 
  };

  const handleUpdateTaskData = async (id, d) => { 
    if (db) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'emssanar_tasks', id), d); 
  };
  ...
  const handleDeleteTask = async (id) => {
    if (db) {
      try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'emssanar_tasks', id)); } 
